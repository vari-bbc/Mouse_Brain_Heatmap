...
# data=data[! data$X1 %in% c( "VS","fiber tracts"),]
# 
# 
     ...
     observeEvent(input$plot.rgn.button,{
        ...
        if(input$lvl == "daughter"){
          rpl = rpl[rpl$strata == input$plot.con & rpl$daughter== input$Region,]
           ...
           rpl = rpl[rpl$strata == input$plot.con & rpl$parent == input$Region,]
        }
        # print(c(colnames(rpl),input$comp))
      
        output$rawpplt <-  renderPlot({p2 <- ggplot(rpl, aes(x = group , y = ZachValue,shape=daughter,color= group,alpha=sex)) +
        geom_boxplot(aes(group=group),outlier.size=-1) +         
        geom_jitter(size=2, height=0,width=.25 ) + 
        facet_grid(~side)+
        scale_color_viridis_d()+
        theme_classic(14) +
        xlab("")+ylab("Value")+scale_alpha_discrete(range=c(0.25,1)) +
