...
# This code breaks Daniella's large rds file, which has all 132 figures into 132 individual feather files for better efficiency
# data = readRDS("/data/home/zachary.madaj/Rprojects/svgData.rds")
# data=data[! data$X1 %in% c( "VS","VL","SEZ","RC","chpl","chfl","IVF","V3","AQ","V4","V4r","c","fiber tracts"),]
# 
# 
...
#   fst::write_fst(tmp[as.numeric(as.character(rownames(tmp))) %% 6 == 1,], paste0('/data/home/zachary.madaj/Rprojects/Allen_Brain_Shiny_App/','data_',i,'.fst'), compress = 70)
# 
# }
# 

# This code is the updated version that takes Daniella's rds files, and turns them into 132 individual feather files for better efficiency
# for(i in 75){
# 
#   data = readRDS(paste0("/data/home/zachary.madaj/Rprojects/Allen_Brain_Shiny_App/",i,"svgFile.rds"))
# 
#   data$X1 = data$region
# 
#   data=data[! data$X1 %in% c( "VS","VL","SEZ","RC","chpl","chfl","IVF","V3","AQ","V4","V4r","c","fiber tracts"),]
...
#   tmp$X1 = gsub("_","",tmp$X1)
#   tmp$X1 = gsub("-","",tmp$X1)
#   # tmp$unique = paste0(tmp$unique,tmp$id)
#   tmp=subset(tmp,select = -c(hemi))
# 
#   fst::write_fst(tmp[as.numeric(as.character(rownames(tmp))) %% 6 == 1,], paste0('/data/home/zachary.madaj/Rprojects/Allen_Brain_Shiny_App/','data_',i,'.fst'), compress = 70)
# }
...
This app is free to use and modify as you (the user) see fit. To reference this app, you can cite the following https://doi.org/10.5281/zenodo.7760581 (webpage link:https://zenodo.org/record/7760581#.ZBsc4dLMJhE). We kindly ask that you make sure The Allen Brain Atlas is cited as the images are supplied from them.
...
process_plots = function(plot.data,fig,sum.lvl,m,k,g,t,l,s){
  ...
  if(s == "Averaged"){
   ...
   if(s == "All" | s == "Averaged"){s = unique(plot.data$sex)}
  ...
  # 
   if(sum.lvl == "Median"){
     ...
     if(!sex.sw){
              ...
              agg.d = aggregate(Proportion ~ genotype +treatment + mpi+marker+X1 + daughter+sex,
       data= agg.d,
       function(x) {median(x,na.rm=T)})
              
       agg.d = aggregate(Proportion ~ genotype +treatment + mpi+marker+X1+sex,
                data= agg.d,
                function(x) {median(x,na.rm=T)})
     } else{
    ...
    raw$mouse = "Mean"
     ...
     raw = subset(plot.data[plot.data$genotype %in% g & 
                 ...
                 plot.data$marker %in% k&
                 plot.data$sex %in% s,],
                 select=c(mouse,X1,Proportion,genotype,treatment,mpi,marker,sex))
   }
   ...
   if(sum.lvl != "Individual"){
        if(sex.sw){
         filler = expand.grid(X1 = unique(tmp$X1[! tmp$X1 %in% raw$X1]), genotype = g,mpi = m, treatment = t, marker = k,mouse = sum.lvl,sex="both")
         } else {
         filler = expand.grid(X1 = unique(tmp$X1[! tmp$X1 %in% raw$X1]), genotype = g,mpi = m, treatment = t, marker = k,mouse = sum.lvl,sex=s)  
         }
         filler$Proportion = NA
         
   } else {
        filler = expand.grid(X1 = unique(tmp$X1[! tmp$X1 %in% raw$X1]), genotype = g,mpi = m, treatment = t, marker = k,mouse = unique(plot.data$mouse),sex=s) 
        filler$Proportion = NA
   }
...
shinyApp(
  ui = fluidPage(
    ...
    # sidebar to demonstrate various slider options ----
      sidebarPanel(
        fileInput("annotation","Please upload ZachTable.csv"),
        
        fileInput("color","If desired, upload heatmapColors.rds here"),
        ...
        selectInput("sex", label="Sexes to Plot",
                         c("All","Averaged",unique(plot.data$sex))),
  ...
  server = function(input, output,session) {
   ...
   observeEvent(input$annotation,{
      ...
      updateSelectInput(session=session,inputId="sex", label="Sexes to Plot",
                         choices=c("All","Averaged",unique(upld.file$sex)))
      
      
      vals$upld.file <- upld.file
      
      vals$lims = c(upld.file$min[1],upld.file$max[1])

  })
    
   observeEvent(input$color,{
      file <- input$color
      vals$mycols <- readRDS(file$datapath)
   })
   
    observeEvent(input$plot.button,{
      print(head(vals$upld.file))
      ss = process_plots(vals$upld.file,input$figure,input$indv,input$mpi,input$mrk,input$geno,input$tx,input$lvl,input$sex)
    ...
    theme_classic(12)+
    coord_fixed() + 
    scale_fill_gradientn(colours=vals$mycols,na.value = "grey50",limits = vals$lims)+ 
    theme(axis.line=element_blank(),
          ...
          if (input$sex == "Averaged"){
            p = p+facet_grid(mouse+mpi+marker+genotype~treatment)
          } else {
            p = p +facet_grid(mouse+mpi+marker+genotype+sex~treatment)
          }
          vals$p <- p 
...
process_plots_multi = function(plot.data,fig,sum.lvl,m,k,g,t,l){
  ...
  plot.data$X1 = gsub("_","",plot.data$X1)
   ...
   for(f in figs){
    ...
    tmp$unique = paste0(tmp$unique,tmp$X1,tmp$id)
    tmp$X1 = gsub("-","",tmp$X1)
    tmp$X1 = gsub("_","",tmp$X1)
    tmp$Figure = rep(f)
    tmpp = rbind(tmp,tmpp)
...
* Users may now select a 'custom' contrast where they pick two groups to analyze. This functionality should really only be used to compare a specific group to the baseline control or reference group (eg mutant_450_6 to WT_0_6).  You can make within strata comparisons  (eg WT vs mutant within the same treatment + marker + MPI), but if there are more than 2 groups you may notice some discrepancies between the 'custom' comparisons and the 'genotype' comparison; that's because when we model  >2 groups (eg a WT, mutant A, mutant B, and double mutant) we get a bit more power by having more accurate variance estimations (all four genotypes in this example contribute to the variance estimates).

* <span style="color: red;"> The analysis method is robust linear regression on rankit transformed outcomes. This method was chosen as there are often strata with no pathology (true zeros) while another comparative strata has mainly non-zeros. This analysis seeks to answer the question, which regions have more pathology.  An alternative method would be zero-inflated beta regression. However, this method is very time-consuming, but potentially more powerful. This app does not implement this method, but it may become available in future versions. Parent regions are included as fixed-effects as opposed to random to save compute time and because there are only a few daughter regions per parent. A mixed-effects model could yield better estimates of the standard error, but for fast and flexible analyses this was a calculated compromise.  For more information on second generation p-values, please view the publications: https://doi.org/10.1080/00031305.2018.1537893 and https://doi.org/10.1371/journal.pone.0188299. Another alternate method is a GAM mixed-effects model to account for spatial correlation across the entire brain; this is the most complicated model, but almost certainly the most powerful as it will have the most complete characterization of the covariance matrix. </span>  


```{r}

rankit = function(t) {qnorm((rank(t)-0.5)/length(t))}

models = function(test,method,strat){
  
  if(any(grepl("WT",fixed=T,test$group))){
    test$group = factor(test$group,levels=c(unique(test$group[grepl("WT",test$group,fixed=T)]),unique(test$group[!grepl("WT",test$group,fixed=T)])))
  }

  if(any(grepl("+/+",fixed=T,test$group))){
    test$group = factor(test$group,levels=c(unique(test$group[grepl("+/+",test$group,fixed=T)]),unique(test$group[!grepl("+/+",test$group,fixed=T)])))
  }
  
  #Adding this here to make an update easier, this is purely convenience to make the code below work
  if(strat == "sex"){
    test$sex = test$group
  }
  # print(head(test))
    ...
    if(method == "Ordinal"){
      if(length(unique(test$daughter))>1){
        if(length(unique(test$sex))>1){
          ...
          if(length(unique(test$sex))>1){
           if(strat == "sex"){
              fit = try(rlm(rankit(ZachValue)~group,data=test,maxit=100))
           } else {
              fit = try(rlm(rankit(ZachValue)~sex+group,data=test,maxit=100))
           }
          } else {
             fit = try(rlm(rankit(ZachValue)~group,data=test,maxit=100))
          }
...
process_p_plots = function(plot.data,fig,p.type,l){
  ...
  # print(head(plot.data))
   if(p.type == "Raw"){
     ...
     plot.data$Significant = ifelse(plot.data$p < 0.05, ifelse(plot.data$estimate <0 ,"Significantly Increased","Significantly Decreased"),"Inconclusive")
     
   } else if(p.type == "FDR") {
     plot.data$Significant = ifelse(plot.data$FDR < 0.05, ifelse(plot.data$estimate <0 ,"Significantly Increased","Significantly Decreased"),"Inconclusive")
   } else {
     plot.data$Significant = ifelse(plot.data$SGPV == 0, ifelse(plot.data$estimate <0 ,"Significantly Increased","Significantly Decreased"),"Inconclusive")
   }
...
shinyApp(
    ...
    # sidebar to demonstrate various slider options ----
      sidebarPanel(
        ...
        selectInput("comp", label="What effect would you like to test?",
                         c("genotype","mpi","treatment","marker","sex","custom")),
        ...
        selectInput(inputId="plotcon", label="Which strata for brain plot",
                         choices=""),
        ...
        actionButton("plot.br.button", "Plot Brain"),
        actionButton("plot.rgn.button", "Plot Region"),
  ...
  server = function(input, output,session) {
    ...
    observeEvent(input$annotation,{
      ...
      upld.file = subset(upld.file,select= - c(X, Region.ID))
      ...
      upld.file$sex = as.character(upld.file$sex)
      vals$upld.file <- upld.file
    ...
    observeEvent(input$lvl,{
      if( length(vals$upld.file)>0){
        if(input$lvl == "daughter"){
            updateSelectInput(session=session,inputId="Region", label="Which region to plot raw data",
                             choices=unique(vals$upld.file$daughter))
            } else {
              
              updateSelectInput(session=session,inputId="Region", label="Which region to plot raw data",
                             choices=unique(vals$upld.file$parent))
            } 
      }
    })
    
    observeEvent(input$comp,{
      if(input$comp == "custom"){
        
        all_combos = unique(tidyr::unite(vals$upld.file, "strata", c("genotype","treatment","mpi","marker"))$strata)
        
        insertUI(selector = "#comp",where = "afterEnd",
           ui = selectInput("cgroup2",label="Please select the second group you'd like to compare", choices = all_combos ))
        
        insertUI(selector = "#comp",where = "afterEnd",
           ui = selectInput("cgroup1",label="Please select the first group you'd like to compare", choices = all_combos ))
         
      } else {
        removeUI(selector = "div:has(>> #cgroup1)")
        removeUI(selector = "div:has(>> #cgroup2)")
      }
    })
    ...
    observeEvent(input$run.button,{
      if(length(vals$upld.file)==0){
        ...
        if(input$comp == "sex"){
          strata = c("genotype","treatment","mpi","marker","sex")
          strata = strata[strata != input$comp]
        } else if(input$comp == "custom"){
        ...
        if(input$comp == "custom"){ 
          upld.file = tidyr::unite(upld.file, "group", strata)
          upld.file = tidyr::unite(upld.file,"strata" ,c("parent","side"))
        } else {
        ...
        if(input$comp != "custom"){ 
          ...
          upld.file = upld.file[upld.file$group %in% c(input$cgroup1, input$cgroup2),]
        }
        ...
        if(length(unique(upld.file$group))<2){
          ...
          if(input$comp == "sex"){
            filt = as.data.frame(table(subset(upld.file,select = c(strata,group))))
          } else {
          ...
          # 
          if(input$comp == "sex"){
            filt.p2 = as.data.frame(table(filt[filt$Freq>2,1]))
          } else {
          ...
          if(input$comp == "sex"){
            # print(head(ix))
            # print(head(upld.file))
            strats = unique(upld.file$strata)[! unique(upld.file$strata) %in% ix]
          } else {
            strats = unique(upld.file$strata)[! unique(upld.file$strata) %in% c(ix)]

          }
          if(input$comp == "custom"){
            removeUI(selector = "div:has(>> #plotcon)")
            upld.file$group = factor(upld.file$group,levels=c(input$cgroup1,input$cgroup2))
          } else {
            updateSelectInput(session=session,inputId="plotcon", label="Which strata to plot brain",
          ...
          updateSelectInput(session=session,inputId="Region", label="Which region to plot raw data",
            ...
            updateSelectInput(session=session,inputId="Region", label="Which region to plot raw data",
                           choices=unique(upld.file$parent))
          } 

          # print(upld.file[upld.file$strata == "Su3_left_WT_3_None",])
          print(input$comp)
          # print(head(strats,20))
          full_results  = rbindlist(lapply(strats, function(x){
            ...
            return(models(upld.file[upld.file$strata == x,],input$method,input$comp))}),use.names=TRUE)
   ...
   observeEvent(input$plot.br.button,{  
      if(length(vals$full_results)>0){
        # print(c(unique(vals$full_results$grouping),input$plotcon,input$fig, input$lvl))
        
        # print(head(vals$full_results))
        if(input$comp == "custom"){
          pl <-process_p_plots(plot.data=vals$full_results,fig=input$fig,input$p.plot,input$lvl)
        } else {
          pl <-process_p_plots(plot.data=vals$full_results[vals$full_results$grouping == input$plotcon  ,],fig=input$fig,input$p.plot,input$lvl)
        }
        ...
        theme_classic(12)+
        coord_fixed() +
        scale_fill_manual(values=c("Inconclusive" = "white","Significantly Increased" = "red","Significantly Decreased" = "blue"),na.value="white")+
        theme(axis.line=element_blank(),
              axis.text.x=element_blank(),
              axis.text.y=element_blank(),
              axis.ticks=element_blank(),
     ...
     observeEvent(input$plot.rgn.button,{
        rpl <- vals$upld.file
        if(input$comp == "sex"){
          strata = c("genotype","treatment","mpi","marker","sex")
          strata = strata[strata != input$comp]
        ...
        if(input$comp == "custom"){ 
          rpl$group = factor(as.character(rpl$strata),levels=c(input$cgroup1,input$cgroup2))
        } else {
        ...
        if(input$lvl == "daughter"){
          if(input$comp == "custom"){
            rpl = rpl[rpl$daughter== input$Region & rpl$group %in% c(input$cgroup1,input$cgroup2),]
          } else {
            rpl = rpl[rpl$strata == input$plotcon & rpl$daughter== input$Region,]
          }
        } else {
          if(input$comp == "custom"){
            rpl = rpl[rpl$parent == input$Region & rpl$group %in% c(input$cgroup1,input$cgroup2),]
          } else {
            rpl = rpl[rpl$strata == input$plotcon & rpl$parent == input$Region,]
          }
        }
        # print(c(colnames(rpl),input$comp))
        output$rawpplt <-  renderPlot({
          if(input$comp == "sex"){
            p2 <- ggplot(rpl, aes(x = group , y = displayValue,shape=daughter,color= group))
          } else {
            p2 <- ggplot(rpl, aes(x = group , y = displayValue,shape=daughter,color= group,alpha=sex))
          }
        ...
        theme_classic(14) +
        xlab("")+ylab("displayValue")+scale_alpha_discrete(range=c(0.25,1)) +
        theme(legend.title = element_blank(),strip.background = element_blank(),
              axis.text = element_text(angle=45,hjust=1))
    
              if(input$lvl == "parent"){
                p2 = p2 + ggtitle(rpl$parent[1])
              } else {
                p2 = p2 + ggtitle(rpl$daughter[1])
              }
              vals$p2 <- p2
